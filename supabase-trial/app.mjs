...
function render(){
  ...
  if(page==='home'){
    ...
    content+=`<section class="panel"><h1>${revealed?(glows.length?'숨겨진 광휘 발견!':'카드를 획득했습니다!'):'카드를 공개해 보세요'}</h1>${revealed&&glows.length?`<p class="glow-discovery">${[...new Set(glows.map(c=>effectNames[effectOf(c)]))].map(esc).join(' · ')}</p><p>카드를 획득했습니다.</p>`:''}<p>카드팩 차감과 카드 저장은 이미 완료되었습니다.</p><div class="${cards.length>1?'multi-reveal':'reveal'}">${revealed?cards.map(c=>cardMarkup(c,1,'reveal')).join(''):'<button class="sealed" data-action="reveal" style="width:100%">✦<br>카드 공개</button>'}</div><button class="secondary" data-action="vault">도감으로</button><button class="secondary" data-action="inventory">카드팩 보관함</button></section>`;
