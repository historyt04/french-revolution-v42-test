...
export function effectLabel(rarity,effect){return effect==='normal'?'일반':effectNames[effect]||'숨겨진 광휘';}
...
export function packView(state,busy=false){
  ...
  <p class="muted">${upgraded?`확률형 팩의 공통 신화 천장 ${state.mythPity}회 (현재 ${state.pity.sinceMyth}회). 확정팩은 해당 등급이 나옵니다.`:'도감 확장 전의 시험 확률이 적용됩니다.'}<br>개봉·천장·중복 방지는 서버가 처리하고, 카드 공개 연출은 기기에서 처리합니다.</p></section>`;
}
export function collectionView(state,rarity='normal',busy=false){
  ...
  const discovered=owned.some(c=>effectOf(c)!=='normal');
  return `<section class="panel collection-panel"><p class="eyebrow">THE CHRONICLE · CARD COLLECTION</p><h1>프랑스혁명 카드 도감</h1><p>등급별 12사건을 모아 나만의 도감을 완성하세요.</p><nav class="tier-tabs" aria-label="카드 등급">${Object.entries(tiers).map(([id,name])=>`<button data-action="tier" data-tier="${id}" class="tierbadge25 ${id}" aria-pressed="${id===rarity}">${name}</button>`).join('')}</nav>
  <div class="collection-progress"><strong>${tiers[rarity]} 사건 ${found} / 12</strong>${discovered?`<span>광휘 완성 ${complete} / 12세트</span>`:''}</div>
  <p class="muted">카드를 누르면 뒷면 해설을 읽을 수 있습니다.${discovered?' 발견한 광휘 중 원하는 모습을 대표 카드로 선택하세요.':''}</p>
  <div class="collection-grid">${catalog.map(e=>{
    const variants=owned.filter(c=>c.eventId===e.eventId),list=['normal',...effects[rarity]].filter(fx=>variants.some(v=>effectOf(v)===fx));
    ...
    return `<article class="collection-slot">${current?cardMarkup(current):`<div class="uncollected"><span>FRANCE · ${esc(e.eventId)}</span><b>✧</b><h2>${esc(e.title)}</h2><p>아직 만나지 못한 사건</p></div>`}${list.length?`<div class="variant-controls" aria-label="${esc(e.title)} 대표 모습">${list.map(fx=>{const c=variants.find(v=>effectOf(v)===fx);return `<button data-action="representative" data-event="${e.eventId}" data-tier="${rarity}" data-effect="${fx}" title="${esc(effectLabel(rarity,fx))}" aria-pressed="${!!current&&effectOf(current)===fx}" ${busy||state.collectionVersion<2?'disabled':''}>${esc(effectLabel(rarity,fx))}<small>${c.quantity}장</small></button>`;}).join('')}</div>`:''}</article>`;
  }).join('')}</div><h2>완성 기록</h2><div class="milestone-list">${(state.milestones||[]).map(m=>`<button class="secondary" data-action="celebrate" data-milestone="${esc(m.id)}">✦ ${esc(milestoneTitle(m.id))}</button>`).join('')||'<p>카드를 꾸준히 모으면 특별한 기록이 열립니다.</p>'}</div><p class="muted">이 시험판에는 12사건 × 5등급이 연결되어 있습니다. 원본 게임의 다른 자료는 삭제하지 않았습니다. 미션·합성·교사 확률 설정 화면은 이전 준비 중입니다.</p></section>`;
}
export function milestoneTitle(id){const [kind,event,rarity]=id.split(':');return kind==='tier'?`${tiers[event]} 카드 도감 완성`:`${catalog.find(c=>c.eventId===event)?.title||event} · ${tiers[rarity]} 카드 수집 완성`;}
export function celebrationView(state,selected=null){
  ...
  if(!m)return '';
  const [kind,event,rarity]=m.id.split(':'),tier=kind==='tier'?event:rarity;
  const cards=kind==='tier'?catalog.map(item=>state.cards.find(c=>c.eventId===item.eventId&&c.rarity===tier&&effectOf(c)===(state.representatives?.[item.eventId+':'+tier]||'normal'))||state.cards.find(c=>c.eventId===item.eventId&&c.rarity===tier)).filter(Boolean):state.cards.filter(c=>c.eventId===event&&c.rarity===tier).sort((a,b)=>['normal',...effects[tier]].indexOf(effectOf(a))-['normal',...effects[tier]].indexOf(effectOf(b)));
  const sentence=kind==='tier'?`${tiers[tier]} 카드 도감을 완성하였습니다.`:`${catalog.find(c=>c.eventId===event)?.title||event} ${tiers[tier]} 카드 수집을 완성하였습니다.`;
  return `<div class="collection-celebration ${kind}" role="dialog" aria-modal="true" aria-label="도감 완성 축하"><section><p class="eyebrow">COLLECTION COMPLETE</p><span class="celebration-star" aria-hidden="true">✦</span><h2>축하합니다!</h2><p class="celebration-sentence">${esc(sentence)}</p><div class="celebration-gallery ${kind}">${cards.map((c,i)=>`<div style="--celebration-index:${i}">${cardMarkup(c,c.quantity||1,'list')}</div>`).join('')}</div><p class="muted">꾸준히 모은 카드가 하나의 기록이 되었습니다.</p><button data-action="milestone-ack" data-milestone="${esc(m.id)}">확인</button></section></div>`;
}
