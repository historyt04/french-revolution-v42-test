...
const icons={book:'M5 5h7l4 3 4-3h7v22h-7l-4 3-4-3H5z M16 8v22 M8 10h4m-4 5h4m-4 5h4m8-10h4m-4 5h4m-4 5h4',
  ...
  crown:'m3 10 7 5 6-10 6 10 7-5-4 16H7z M8 30h16',
  sword:'m5 28 17-17 7-8-3 10L9 30 M6 20l8 8 M3 26l4 4',
  clock:'M16 5v4 M12 3h8 M16 16l6-5 M27 18a11 11 0 1 1-22 0 11 11 0 1 1 22 0',
  cards:'M5 8h15v21H5z M12 3h15v21 M9 13h7m-7 5h7m-7 5h7',
  link:'m12 20 8-8 M13 8l3-3a6 6 0 0 1 9 9l-3 3 M10 15l-3 3a6 6 0 0 0 9 9l3-3',
  map:'m3 7 8-3 10 4 8-3v23l-8 3-10-4-8 3z M11 4v23 M21 8v23',
...
export function recordsView(state){return `<section class="panel"><p class="eyebrow">MY LEARNING JOURNAL</p><h1>나의 학습 기록</h1><p class="muted">서버에 저장된 최근 10회의 초급 완료 기록입니다.</p>${state.runs.map(r=>`<article class="record"><strong>초급 ${r.correctCount}/${r.totalCount} 완료</strong><p>오답 ${r.errorCount}회 · ${(r.elapsedMs/1000).toFixed(1)}초<br><small>${esc(new Date(r.completedAt).toLocaleString('ko-KR'))} · ${r.rewardPacks?'일반 카드팩 1개 지급':'보상 수령 후 재학습'}</small></p></article>`).join('')||'<p>아직 완료 기록이 없습니다.</p>'}<button class="secondary" data-action="refresh">기록 새로 확인</button></section>`;}
function practiceHeader(title,subtitle){return `<div class="practice-heading"><p class="eyebrow">1789 — 1799 · LOCAL PRACTICE</p><h1>${title}</h1><p>${subtitle}</p><span class="practice-badge">연습용 · 카드팩 지급과 서버 저장 없음</span></div>`;}
...
export function memoryView(s){
  ...
  const card=(e,placed=false)=>`<button class="memory-card ${s.selected===e.id?'selected':''} ${s.checked&&placed?(s.placed[e.id]===e.zone?'correct':'incorrect'):''}" data-action="memory-select" data-event="${e.id}" ${s.phase!=='play'?'disabled':''} aria-pressed="${s.selected===e.id}"><small>FRANCE · ${e.id}</small><strong>${esc(e.title)}</strong>${show?`<span>${esc(e.date)}</span>`:''}</button>`;
  return `${practiceHeader('기억도전','사라진 혁명 지도 · 사건을 알맞은 시기에 놓아 보세요.')}<section class="parchment practice-toolbar"><strong>${s.phase==='ready'?'지도를 5초 동안 기억하세요':s.phase==='memorize'?`기억할 시간 ${seconds}초`:s.phase==='done'?'혁명 지도 완성!':show?`힌트 ${seconds}초`:'카드 선택 → 시기 선택'}</strong><span>배치 ${Object.keys(s.placed).length}/12 · 힌트 ${s.hints}회</span><button class="text-button" data-action="practice-exit">나가기</button></section>
  ${s.phase==='ready'?'<section class="parchment memory-intro"><h2>순서보다 흐름을 기억하세요</h2><p>처음 5초 동안 사건이 어느 시기에 있는지 확인합니다.<br>카드를 누른 뒤 시기 칸의 “여기에 놓기”를 누르세요.<br>같은 시기 안에서는 정확한 순서를 맞힐 필요가 없습니다.</p><button data-action="memory-begin">5초 기억 시작</button></section>':`<section class="memory-map">${zones.map((z,i)=>`<article class="era era-${i}"><header><span>0${i+1}</span><h2>${z.title}</h2><strong>${z.period}</strong><p>${z.note}</p></header><div class="era-cards">${(show?events.filter(e=>e.zone===i):events.filter(e=>s.placed[e.id]===i)).map(e=>card(e,true)).join('')}</div>${s.phase==='play'&&!show?`<button class="place-button" data-action="memory-place" data-zone="${i}" ${!s.selected?'disabled':''}>${s.selected?'선택한 카드 여기에 놓기':'카드를 먼저 선택하세요'}</button>`:''}</article>`).join('')}</section>${s.phase==='play'&&!show?`<section class="parchment card-tray"><h2>아직 배치하지 않은 사건</h2><div>${s.order.filter(e=>s.placed[e.id]===undefined).map(e=>card(e)).join('')||'<p>모두 배치했어요. 위치를 확인해 보세요.</p>'}</div></section>`:''}`}
  <div class="practice-controls"><p role="status">${esc(s.feedback||'연습용입니다. 힌트를 사용해도 괜찮아요.')}</p>${s.phase==='play'?`<button class="secondary" data-action="memory-hint" ${show?'disabled':''}>3초 힌트</button><button data-action="memory-check" ${show?'disabled':''}>배치 확인</button>`:s.phase==='done'?'<button data-action="memory">다시 연습</button>':''}<button class="secondary" data-action="practice-exit">학습 홈으로</button></div>`;
}
