...
const pending2=m=>(state42?.pendingAttempts||[]).find(a=>a.mode===m);
...
function decorateStage2Body(h){
 ...
 const resume=pending&&!active2(view)?`<div class="resume-banner2"><b>진행 중인 ${label2[view]} 게임이 있습니다.</b><button class="btn" onclick="offerResume2('${view}')">이어하기·처음부터 선택</button></div>`:'';
 ...
 return matchingTitle+title+reward+resume+(active2(view)?`<div class="pause-actions2"><button class="btn alt" onclick="stopGame2('${view}')">중단·저장</button></div>`:'')+h;
}
...
function stopGame2(mode,nextView){if(!active2(mode))return;stage2Dialog={kind:'pause',mode,nextView};render42()}
...
begin42=async function(mode,options={}){await refresh42();const pending=pending2(mode);if(pending&&!options.restart&&!options.resume){stage2Dialog={kind:'resume',mode,options};render42();return null}view=mode;return beginStage1(mode,options)};
...
const goStage1=go;
go=function(v){if(active2(view)&&v!==view&&!leaving2){stopGame2(view,v);return}goStage1(v);if(modes2.includes(v)&&pending2(v)&&!active2(v))offerResume2(v)};
...
queueDraft42=function(m){if(!['advanced','challenge','speedrun','baitrun','faceoff'].includes(m)||!active2(m))return;const snapshot=slots2(m);if(!snapshot)return;try{sessionStorage.setItem('stage2-draft:'+attempts42[m].attemptId,JSON.stringify({revision:attempts42[m].state.revision,slots:snapshot}))}catch{}clearTimeout(draft42);draft42=setTimeout(()=>{if(busy42)return queueDraft42(m);const slots=clone42(slots2(m));draftPending42=draftPending42.then(async()=>{try{const out=await api42.request('attempt.save',payload42(m,{slots}));attempts42[m]=out;lastDraft2[m]=JSON.stringify(slots);if(JSON.stringify(slots2(m))!==lastDraft2[m])queueDraft42(m);else sessionStorage.removeItem('stage2-draft:'+out.attemptId)}catch(e){toast42(e.message)}})},400)};
function observeDraft2(){if(!active2(view))return;const slots=slots2(view);if(slots&&JSON.stringify(slots)!==lastDraft2[view])queueDraft42(view)}
...
// Alert before losing edits; completed server actions already persist each quiz/matching step.
addEventListener('beforeunload',e=>{if(active2(view)){e.preventDefault();e.returnValue='';}});
